...
def run_gmap(ref_mono, in_cds, allele_cnt, workdir, threads):
    ...
    if not os.path.exists("CpDB"):
        cmd = "gmap_build -D . -d CpDB %s &> gmap_build.log" % ref_mono
    ...
    if not os.path.exists("gmap.gff3"):
        ...
        if fs >= 2 ** 32:
            cmd = "gmapl -D . -d CpDB -f 2 -n %s -t %s %s 1> gmap.gff3 2>gmap.err" % (allele_cnt, threads, in_cds)
        else:
            cmd = "gmap -D . -d CpDB -f 2 -n %s -t %s %s 1> gmap.gff3 2>gmap.err" % (allele_cnt, threads, in_cds)
